...
// ==========================================
// FAST LOCAL FOOTBALLER CACHED DATABASE SYSTEM
// ==========================================
...
let footballerCache: Footballer[] = [];
const DB_PATH = path.join(process.cwd(), "footballer-db.json");
...
function saveDb() {
  try {
    fs.writeFileSync(DB_PATH, JSON.stringify(footballerCache, null, 2), "utf-8");
...
function checkFuzzyMatch(array: string[], query: string): boolean {
...
function checkPlayerCriteria(player: Footballer, type: string, value: string): boolean {
...
function findFootballerInCache(name: string): Footballer | null {
  const cleanName = name.toLowerCase().trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9\s]/g, "");
  if (!cleanName) return null;
  
  // 1. Fully-matching name or synonym
  ...
  // 2. Part-name matching (e.g. "Lampard" matching "Frank Lampard")
...
const TENABLE_CURATED_TOPICS = [
  ...
  },
  {
    ...
    items: [
      "Reggiana", "Parma", "Juventus", "AC Milan", "Chelsea", "Paris Saint-Germain", "Real Madrid", "Bayern Munich", "Napoli", "Everton", "Barcelona", "Manchester City"
    ]
  },
  {
    id: "t9",
    ...
    items: [
      "Ryan Giggs", "Paul Scholes", "Gary Neville", "Denis Irwin", "Roy Keane", "David Beckham", "Phil Neville", "Nicky Butt", "Ole Gunnar Solskjaer", "John Terry", "Petr Cech", "Didier Drogba", "Ferdinand", "Wayne Rooney", "Sergio Aguero", "Fernandinho", "Ederson", "Kevin De Bruyne", "Phil Foden", "Bernardo Silva", "Riyad Mahrez"
    ]
  },
  {
    id: "t10",
    ...
    items: [
      "England", "France", "Germany", "Italy", "Netherlands", "Portugal", "Spain", "Soviet Union", "Croatia", "Czech Republic", "Czechoslovakia", "Denmark", "Hungary", "Northern Ireland", "Scotland", "Ukraine", "Bulgaria"
    ]
  },
  {
    id: "t11",
    ...
    items: [
      "Real Madrid", "AC Milan", "Bayern Munich", "Liverpool", "Barcelona", "Ajax", "Inter Milan", "Manchester United", "Juventus", "Benfica", "Chelsea", "Nottingham Forest", "Porto", "Celtic", "Hamburg", "Feyenoord", "Aston Villa", "PSV Eindhoven", "Red Star Belgrade", "Marseille", "Borussia Dortmund", "Manchester City"
    ]
  },
  {
    id: "t12",
    ...
    items: [
      "Cristiano Ronaldo", "Lionel Messi", "Robert Lewandowski", "Ali Mabkhout", "Romelu Lukaku", "Sunil Chhetri", "Neymar", "Luis Suarez", "Harry Kane", "Edin Dzeko", "Mitrovic", "Alexis Sanchez", "Eran Zahavi", "Sadio Mane", "Kylian Mbappe"
    ]
  },
  {
    id: "t13",
    ...
    items: [
      "Luis Figo", "Ronaldo Nazario", "Michael Laudrup", "Luis Enrique", "Samuel Eto'o", "Javier Saviola", "Gheorghe Hagi", "Robert Prosinecki", "Albert Celades", "Alfonso Perez", "Bernd Schuster", "Julen Lopetegui"
    ]
  },
  {
    id: "t14",
    ...
    items: [
      "Ryan Giggs", "Cesc Fabregas", "Wayne Rooney", "Kevin De Bruyne", "Frank Lampard", "Dennis Bergkamp", "Steven Gerrard", "James Milner", "David Beckham", "Teddy Sheringham"
    ]
  },
  {
    id: "t15",
    ...
    items: [
      "Alfredo Di Stefano", "Raymond Kopa", "Luis Figo", "Ronaldo Nazario", "Fabio Cannavaro", "Cristiano Ronaldo", "Luka Modric", "Karim Benzema", "Zinedine Zidane", "Michael Owen"
    ]
  },
  {
    id: "t16",
    ...
    items: [
      "Cristiano Ronaldo", "Lionel Messi", "Robert Lewandowski", "Karim Benzema", "Thomas Muller", "Raul Gonzalez", "Zlatan Ibrahimovic", "Thierry Henry", "Filippo Inzaghi", "Harry Kane"
    ]
  },
  {
    id: "t17",
    ...
    items: [
      "Ajax", "Benfica", "Porto", "Celtic", "Feyenoord", "PSV Eindhoven", "Steaua Bucuresti", "Red Star Belgrade"
    ]
  },
  {
    id: "t18",
    ...
    items: [
      "Hugo Lloris", "Lilian Thuram", "Olivier Giroud", "Thierry Henry", "Marcel Desailly", "Antoine Griezmann", "Zinedine Zidane", "Patrick Vieira", "Didier Deschamps", "Laurent Blanc"
    ]
  },
  {
    id: "t19",
    ...
    items: [
      "Jose Mourinho", "Carlo Ancelotti", "Antonio Conte", "Thomas Tuchel", "Guus Hiddink", "Roberto Di Matteo", "Claudio Ranieri", "Maurizio Sarri", "Frank Lampard", "Gianluca Vialli"
    ]
  },
  {
    id: "t20",
    ...
    items: [
      "Juventus", "AC Milan", "Inter Milan", "Roma", "Lazio", "Napoli", "Fiorentina", "Atalanta", "Torino", "Bologna"
    ]
  }
];

const CAREER_CURATED_PLAYERS = [
  {
    id: "p1",
...
const TENABLE_MUSIC_CURATED_TOPICS = [
  {
    id: "m_t1",
  ...
  },
  {
    ...
    items: [
      "The Beatles", "Queen", "Pink Floyd", "Led Zeppelin", "Coldplay", "The Rolling Stones", "Oasis", "The Who", "Radiohead", "Black Sabbath"
    ]
  },
  {
    id: "m_t4",
    ...
    items: [
      "Michael Jackson", "Beyonce", "Harry Styles", "Justin Timberlake", "Paul McCartney", "John Lennon", "Phil Collins", "George Michael", "Sting", "Robbie Williams"
    ]
  },
  {
    id: "m_t5",
    ...
    items: [
      "The Beatles", "Mariah Carey", "Rihanna", "Michael Jackson", "Madonna", "Taylor Swift", "Drake", "Stevie Wonder", "Janet Jackson", "Whitney Houston"
    ]
  },
  {
    id: "m_t6",
    ...
    items: [
      "Daft Punk", "Avicii", "Calvin Harris", "David Guetta", "Armin van Buuren", "Tiesto", "Skrillex", "Kygo", "Marshmello", "Deadmau5"
    ]
  },
  {
    id: "m_t7",
    ...
    items: [
      "Elton John", "Bob Dylan", "Paul McCartney", "David Bowie", "Stevie Wonder", "Prince", "Bruce Springsteen", "John Lennon", "Neil Young", "Joni Mitchell"
    ]
  },
  {
    id: "m_t8",
    ...
    items: [
      "Georg Solti", "Quincy Jones", "Beyonce", "Pierre Boulez", "Stevie Wonder", "U2", "John Williams", "Jay-Z", "Kanye West", "Aretha Franklin", "Taylor Swift"
    ]
  },
  {
    id: "m_t9",
    ...
    items: [
      "Madonna", "Whitney Houston", "Mariah Carey", "Celine Dion", "Janet Jackson", "Britney Spears", "Lady Gaga", "Beyonce", "Rihanna", "Taylor Swift", "Ariana Grande"
    ]
  },
  {
    id: "m_t10",
    ...
    items: [
      "Eminem", "Tupac Shakur", "The Notorious B.I.G.", "Jay-Z", "Kanye West", "Kendrick Lamar", "Snoop Dogg", "Lil Wayne", "Dr. Dre", "Drake"
    ]
  },
  {
    id: "m_t11",
    ...
    items: [
      "The Beatles", "The Rolling Stones", "Pink Floyd", "U2", "Queen", "Coldplay", "Radiohead", "Red Hot Chili Peppers", "Foo Fighters", "Green Day"
    ]
  },
  {
    id: "m_t12",
    ...
    items: [
      "Thriller", "Back in Black", "The Dark Side of the Moon", "The Bodyguard", "Rumours", "Saturday Night Fever", "Led Zeppelin IV", "Come On Over", "Bad", "Falling into You"
    ]
  }
];

const CAREER_MUSIC_CURATED_PLAYERS = [
  {
    id: "m_p1",
    name: "The Beatles",
  ...
  },
  {
    ...
    birthYear: "1991",
    clues: ["Famous for performing solo with just an acoustic guitar and a loop pedal", "His math-themed albums (+, x, ÷, =, -) are record-breaking worldwide successes", "His hit 'Shape of You' became one of the most-streamed songs on Spotify history"],
    career: [
      ...
      { years: "2017", club: "÷ (Divide) & Shape of You", apps: "Shape of You / Perfect", goals: "Record-Breaking Billboard Runs" },
...
// ==========================================
// MOVIES GAME CURATED DATA POOLS
// ==========================================
...
const TENABLE_MOVIES_CURATED_TOPICS = [
  {
    id: "f_t1",
  ...
  },
  {
    ...
    items: [
      "Marvel Cinematic Universe", "Star Wars", "Harry Potter", "Spider-Man", "James Bond", "Middle-earth", "Fast and Furious", "DC Extended Universe", "Jurassic Park", "Batman"
    ]
  },
  {
    id: "f_t6",
    ...
    items: [
      "Taxi Driver", "Raging Bull", "Goodfellas", "Casino", "The Departed", "The Wolf of Wall Street", "Shutter Island", "Gangs of New York", "Killers of the Flower Moon", "The Aviator"
    ]
  },
  {
    id: "f_t7",
    ...
    items: [
      "Avatar", "Titanic", "Avatar: The Way of Water", "The Terminator", "Terminator 2: Judgment Day", "Aliens", "The Abyss", "True Lies", "Piranha II", "Strange Days"
    ]
  },
  {
    id: "f_t8",
    ...
    items: [
      "Iron Man", "The Avengers", "Captain America: The Winter Soldier", "Guardians of the Galaxy", "Captain America: Civil War", "Black Panther", "Avengers: Infinity War", "Avengers: Endgame", "Spider-Man: No Way Home", "Iron Man 3"
    ]
  },
  {
    id: "f_t9",
  ...
  },
  {
    ...
    items: [
      "Adam West", "Michael Keaton", "Val Kilmer", "George Clooney", "Christian Bale", "Ben Affleck", "Robert Pattinson", "Will Arnett", "Kevin Conroy", "Diedrich Bader"
    ]
  },
  {
    id: "f_t11",
    ...
    items: [
      "The Lion King", "Frozen II", "The Super Mario Bros. Movie", "Frozen", "Incredibles 2", "Minions", "Toy Story 4", "Toy Story 3", "Despicable Me 3", "Finding Dory"
    ]
  },
  {
    id: "f_t12",
    ...
    items: [
      "Avatar", "Avatar: The Way of Water", "Inception", "Interstellar", "The Matrix Reloaded", "Gravity", "The Martian", "Dune: Part Two", "Dune", "Arrival"
    ]
  }
];

const CAREER_MOVIES_CURATED_PLAYERS = [
  {
    id: "f_p1",
...
app.post("/api/tic-tac-toe/create", async (req, res) => {
  ...
  if (solvable) {
    if (theme === "music") {
      ...
      // Solvable grid: Pick one from our curated certified list randomly
  ...
  // Not guaranteed solvable / Random Grid
  if (!ai) {
    if (theme === "music") {
      ...
      // If no gemini key, just generate randomly from pool offline
      ...
      // Keep generating to avoid rows/cols sharing same values
...
- "Partner" (e.g. Played with Messi, Played with Ronaldo, Played with Ibrahimovic, Played with Hazard)`;
    ...
    // Generate chaotic random grid via Gemini
...
// Helper function for robust offline verification of players
function runOfflineVerify(playerName: string, rowCriteria: any, colCriteria: any, theme: string) {
  ...
  if (theme === "music") {
    // Basic demo music names with structured profiles
    ...
    const getMusicProfile = (name: string) => {
    ...
    const checkMusicCriteria = (prof: any, crit: any): boolean => {
      const t = crit.type.toLowerCase();
      const v = crit.value.toLowerCase();
      if (t === 'genre') return prof.genres.some((g: string) => v.includes(g) || g.includes(v));
      if (t === 'nationality') return v.includes(prof.nationality) || prof.nationality.includes(v);
      if (t === 'award') return prof.awards.some((a: string) => v.includes(a));
      if (t === 'chart') return prof.charts.some((c: string) => v.includes(c));
    ...
    return {
      ...
      clarification: totalSuccess
        ? `Verified (Offline): "${playerName}" satisfies: ${rowCriteria.value} and ${colCriteria.value}.`
        : `Reversed: "${playerName}" does not satisfy both criteria offline.`
    };

  } else if (theme === "movies") {
    // Movies offline fallback
    ...
    const getMovieProfile = (name: string) => {
      const n = name.toLowerCase();
    ...
    const checkMovieCriteria = (prof: any, crit: any): boolean => {
      const t = crit.type.toLowerCase();
      const v = crit.value.toLowerCase();
      if (t === 'genre') return prof.genres.some((g: string) => v.includes(g) || g.includes(v));
      if (t === 'director') {
        if (prof.type === 'director') return v.includes(prof.directorMatched?.[0] || '');
        return prof.directorMatched?.some((d: string) => v.includes(d)) || false;
      }
      if (t === 'award') return prof.awards.some((a: string) => v.includes(a));
      if (t === 'stat' && v.includes('1 billion')) return prof.films?.some((f: string) => ['avatar', 'titanic', 'inception', 'toy story', 'the dark knight', 'dark knight'].includes(f)) || false;
      return false;
    };
    ...
    // Football offline fallback with precise keyword lists
    ...
    const isMatch = (array: string[], name: string) => {
      return array.some(kw => name.includes(kw));
    };

    const getClubList = (val: string) => {
    ...
    const getNationList = (val: string) => {
      const v = val.toLowerCase();
    ...
    const getTrophyList = (val: string) => {
      const v = val.toLowerCase();
    ...
    return {
      success: totalSuccess,
      clarification: totalSuccess 
        ? `Verified (Offline): Found "${playerName}" meeting both criteria!` 
        : `Referee check failed: "${playerName}" does not satisfy the required criteria for this position (${rowCriteria.value} + ${colCriteria.value}).`
    };
  }
}
...
app.post("/api/tic-tac-toe/verify", async (req, res) => {
  ...
  // 1. FAST CACHED DATABASE SEARCH (Only for Football theme)
  if (theme === "football") {
    const cachedPlayer = findFootballerInCache(playerName);
    if (cachedPlayer) {
  ...
  if (false) {
    ...
    if (theme === "music") {
      ...
      const checkMusicCriteria = (art: any, crit: any): boolean => {
        const t = crit.type.toLowerCase();
        const v = crit.value.toLowerCase();
        if (t === 'genre') return art.genres.some((g: string) => v.includes(g) || g.includes(v));
        ...
        if (t === 'award') return art.awards.some((a: string) => v.includes(a));
        if (t === 'chart') return art.charts.some((c: string) => v.includes(c));
        return false;
      };
      ...
      return res.json({
        success: totalSuccess,
        clarification: totalSuccess 
          ? `Verified (Offline): "${playerName}" matches both: ${rowCriteria.value} and ${colCriteria.value}.` 
          : `Reversed: "${playerName}" does not satisfy both criteria offline.`
      });

    } else if (theme === "movies") {
      // Basic demo films / stars / directors with structured profiles
      ...
      // Try local cache check first, even if offline
      const cachedPlayer = findFootballerInCache(playerName);
      let matchRow = false;
      let matchCol = false;
      ...
      if (cachedPlayer) {
        ...
        // Fall back to hardcoded keyword list evaluation
        if (rowCriteria.type === 'Club') {
          const arr = getClubList(rowCriteria.value);
          matchRow = arr ? isMatch(arr, lowerName) : false;
        } else if (rowCriteria.type === 'Nationality') {
  ...
  try {
    ...
    // Expand our database with this newly fetched profile on success!
    if (theme === "football" && verifyData.success && verifyData.foundPlayerProfile) {
      ...
      if (existing) {
        // Update existing profile with new fetched facts to make the cache self-healing!
    ...
    const correctName = (verifyData.success && verifyData.foundPlayerProfile?.name)
      ? verifyData.foundPlayerProfile.name
      : playerName;
...
app.post("/api/tenable/create", async (req, res) => {
  ...
  if (!customTopicRequest && !ai) {
    // Pick standard pre-made topic
    const idx = Math.floor(Math.random() * curatedPool.length);
    return res.json(curatedPool[idx]);
  }

  if (!ai) {
    // Offline mode but requested custom - give simple warning and a curated one
    const idx = Math.floor(Math.random() * curatedPool.length);
  ...
  try {
    // Use Gemini to generate a fully custom or a highly immersive random tenable topic of exactly 10 answers!
    let promptTheme = "";
    ...
    Return a strict JSON format matching:
    {
      ...
      "items": string[] (Array of exactly 10 correct answers. Keep the strings as standard titles / names, e.g., ["Sergio Agüero", "Luis Suárez", "Carlos Tevez"...])
    }`;
    ...
    return res.json({
      ...
      items: data.items.slice(0, 10) // Limit to exactly 10 elements
    });
...
// Tenable verification endpoint (helps with soft matching, synonyms, etc. using Gemini)
app.post("/api/tenable/verify-guess", async (req, res) => {
  ...
  // Offline basic direct/substring matching:
  const lowerGuess = guess.toLowerCase().trim().normalize("NFD").replace(/[\u0300-\u036f]/g, ""); // strip accents
  
  // Try offline match first (fastest)
  const matchedIndex = correctItems.findIndex(item => {
    const normItem = item.toLowerCase().trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    
    // Check if guess equals the item, or is a very close match (split names)
    if (normItem === lowerGuess) return true;
    
    // e.g. "Cristiano Ronaldo" matches "Ronaldo" (only if item is Ronaldo/Cristiano Ronaldo)
    const itemParts = normItem.split(" ");
    const guessParts = lowerGuess.split(" ");
    
    // Check if last name is matched and length is key
    if (guessParts.length === 1 && itemParts.length > 1) {
      // Must be a safe surname match, e.g., "Shearer" matches "Alan Shearer", "Haaland" matches "Erling Haaland"
  ...
  // If online, use Gemini for fuzzy/synonym spelling check to be very forgiving and fun!
  if (!ai) {
...
app.post("/api/career/create", async (req, res) => {
  ...
  if (!customRequest) {
    // Pick standard pre-made famous entity/player
    const idx = Math.floor(Math.random() * curatedPool.length);
    return res.json(curatedPool[idx]);
  }

  if (!ai) {
    // Offline path with custom player requested: Give warning and serve curated random
    const idx = Math.floor(Math.random() * curatedPool.length);
    const selected = { ...curatedPool[idx] };
  ...
  try {
    ...
    if (theme === "music") {
      ...
      Choose a highly famous player (e.g. Virgil van Dijk, Kevin De Bruyne, Robert Lewandowski, Kylian Mbappé, Mohamed Salah, Karim Benzema, Thomas Müller, etc.) to ensure the career is identifiable.
...
app.post("/api/career/verify-guess", async (req, res) => {
  ...
  // Direct check for last name or full first+last
...
function readLeaderboards() {
  try {
    if (fs.existsSync(LEADERBOARD_FILE)) {
      ...
      // Create with seeds
      fs.writeFileSync(LEADERBOARD_FILE, JSON.stringify(L_SEED_DATA, null, 2), "utf-8");
      return L_SEED_DATA;
    }
  } catch (error) {
...
function writeLeaderboards(data: any) {
  try {
...
app.post("/api/leaderboards/submit", (req, res) => {
  ...
  const newEntry = {
    ...
    tttSolvable: entry.tttSolvable !== undefined ? !!entry.tttSolvable : true,
    ...
    completionTimeSeconds: entry.completionTimeSeconds ? Number(entry.completionTimeSeconds) : undefined,
  };
...
app.get("/api/leaderboards/list", (req, res) => {
  ...
  if (gameType) {
    res.json(list.filter((e: any) => e.gameType === gameType));
  } else {
    res.json(list);
  }
});
...
// ==========================================
// DEEP DOUBLE-CHECK REF VERIFICATION (DISPUTES)
// ==========================================
  ...
  if (!ai) {
    if (gameType === "tic-tac-toe") {
      ...
      return res.json({
        ...
        proof: offlineResult.success
          ? `⭐ VAR Review Successful (Offline Fallback): Checked our local archives and confirmed that "${actualPlayerName}" successfully satisfies both "${rowCriteria.value}" and "${colCriteria.value}" requirements!`
          : `Dispute Rejected (Offline Fallback): Checked our local archives, but we could not verify that "${playerName}" satisfies both "${rowCriteria.value}" and "${colCriteria.value}".`
      });
    } 
      ...
      const match = correctItems.find((item: string) => {
        const normItem = item.toLowerCase().trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              actualCorrect: { type: Type.BOOLEAN },
              correctedValue: { type: Type.STRING },
              proof: { type: Type.STRING }
            }
          }
        }
...
// ==========================================
// THE FOOTBALL GAME / TOPIC CARD BLITZ ROUTES
// ==========================================
...
const FOOTBALL_CARDS = [
  ...
  { id: "fc-12", title: "French players playing in the Bundesliga", category: "Nationality", hint: "Think Kingsley Coman, Dayot Upamecano, Benjamin Pavard, Franck Ribéry." },
...
const MUSIC_CARDS = [
  ...
  { id: "mc-5", title: "Famous solo artists who previously left a massive band", category: "History", hint: "Think Harry Styles, Robbie Williams, Michael Jackson, Beyoncé, Phil Collins, Zayn Malik." }
];
...
app.post("/api/card-game/generate-card", async (req, res) => {
  ...
  // If a custom prompt is provided with AI
  if (customPrompt && ai) {
    try {
...
app.post("/api/card-game/verify-answer", async (req, res) => {
  ...
  // 1. Check duplicates
  const isDuplicate = alreadyGuessed.some((g: string) => {
